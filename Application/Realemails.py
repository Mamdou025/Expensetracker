...
def create_html_file(email_info, output_dir, index):
    """Create an HTML file for an email."""
    try:
...
<body>
    <div class="email-header">
        ...
        <div class="metadata">
            ...
            <span class="{'transaction-badge' if email_info['is_transaction'] else 'non-transaction-badge'}">
                {'💳 TRANSACTION' if email_info['is_transaction'] else '📋 NON-TRANSACTION'}
            </span>
            
            <strong>Matched Keyword:</strong> 
            <span>{email_info['matched_keyword'] or 'None'}</span>
...
def create_master_index(output_dir, all_emails, config):
    """Create a comprehensive index HTML file for all emails."""
    try:
        index_path = os.path.join(output_dir, "index.html")
        
        transaction_emails = [e for e in all_emails if e['is_transaction']]
        non_transaction_emails = [e for e in all_emails if not e['is_transaction']]
        ...
        for email_info in all_emails:
            ...
            if bank not in bank_stats:
                bank_stats[bank] = {'total': 0, 'transactions': 0}
            bank_stats[bank]['total'] += 1
            if email_info['is_transaction']:
                bank_stats[bank]['transactions'] += 1
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Email Export Master Index</title>
    <style>
        body {{
...
<body>
    <div class="header">
        <h1>📧 Multi-Bank Email Export</h1>
        <p>Comprehensive email analysis from all configured banks</p>
    </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div>Transaction Emails</div>
        </div>
        <div class="stat-card">
            <div class="stat-number">{len(non_transaction_emails)}</div>
            <div>Non-Transaction</div>
        </div>
        <div class="stat-card">
            <div class="stat-number">{len(bank_stats)}</div>
            <div>Banks Processed</div>
        </div>
    </div>
    
    <div class="controls">
        <div class="filter-buttons">
            <button class="filter-btn active" onclick="filterEmails('all')">All Emails</button>
            <button class="filter-btn" onclick="filterEmails('transaction')">Transactions Only</button>
            <button class="filter-btn" onclick="filterEmails('non-transaction')">Non-Transactions</button>"""
        
        # Add bank filter buttons
        for bank_name in bank_stats.keys():
            bank_display = bank_name.replace('_', ' ').title()
            html_content += f"""
            <button class="filter-btn" onclick="filterEmails('{bank_name}')">{bank_display}</button>"""
        
        html_content += f"""
        </div>
        <input type="text" class="search-box" placeholder="Search by subject, sender, or content..." onkeyup="searchEmails()" id="searchInput">
    </div>
    ...
    <table id="emailTable">
        ...
        # Add email rows
        for email_info in all_emails:
            row_class = "transaction" if email_info['is_transaction'] else "non-transaction"
            badge_class = "transaction-badge" if email_info['is_transaction'] else "non-transaction-badge"
            badge_text = "💳 TRANSACTION" if email_info['is_transaction'] else "📋 NON-TRANSACTION"
            
            bank_display = email_info['bank_name'].replace('_', ' ').title()
            
            html_content += f"""
            <tr class="{row_class}" data-type="{'transaction' if email_info['is_transaction'] else 'non-transaction'}" data-bank="{email_info['bank_name']}">
                <td><span class="badge {badge_class}">{badge_text}</span></td>
                <td><span class="bank-badge">{bank_display}</span></td>
                <td>{email_info['datetime']}</td>
                <td>{email_info['subject'][:60]}{'...' if len(email_info['subject']) > 60 else ''}</td>
                <td>{email_info['matched_keyword'] or 'None'}</td>
                <td><a href="{email_info['filename']}" target="_blank" class="email-link">View Email</a></td>
            </tr>"""
    ...
    <script>
        function filterEmails(type) {{
            ...
            rows.forEach(row => {{
                if (type === 'all') {{
                    row.style.display = '';
                }} else if (type === 'transaction' || type === 'non-transaction') {{
    ...
    <div style="margin-top: 30px; padding: 20px; background: white; border-radius: 8px; text-align: center;">
        ...
        for bank_name, stats in bank_stats.items():
            bank_display = bank_name.replace('_', ' ').title()
            html_content += f"""
        <p><strong>{bank_display}:</strong> {stats['total']} emails ({stats['transactions']} transactions)</p>"""
    ...
    try:
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                # Classify the email
                is_transaction, matched_bank, matched_keyword = classify_email(
...
def fetch_all_bank_emails():
    """Fetch emails from ALL banks configured in the YAML file."""
    try:
        # Get credentials
        ...
        print(f"🔍 Processing emails from {len(config['banks'])} banks:")
        for bank_name, bank_config in config['banks'].items():
        ...
        for bank_name, bank_config in config['banks'].items():
            ...
            for email_id in mail_id_list:
                ...
                if email_info:
                    ...
                    status = "✅ TRANSACTION" if email_info['is_transaction'] else "📋 NON-TRANSACTION"
                    print(f"   {status}: {email_info['subject'][:40]}... → {filename}")
        ...
        # Create master index
        if all_emails:
            index_path = create_master_index(output_dir, all_emails, config)
            
            print(f"\n{'='*80}")
            print(f"📊 MULTI-BANK EMAIL EXTRACTION COMPLETE")
            print(f"{'='*80}")
            print(f"📁 Output directory: {os.path.abspath(output_dir)}")
            print(f"📈 Total emails extracted: {len(all_emails)}")
            print(f"💳 Transaction emails: {len([e for e in all_emails if e['is_transaction']])}")
            print(f"📋 Non-transaction emails: {len([e for e in all_emails if not e['is_transaction']])}")
            if index_path:
            ...
            for bank, count in bank_counts.items():
                print(f"   {bank.replace('_', ' ').title()}: {count} emails")
...
if __name__ == "__main__":
    ...
    print(f"🚀 MULTI-BANK EMAIL EXTRACTOR")
    print(f"=" * 50)
    print(f"📅 Date Range: {START_DATE} to {END_DATE}")
    print(f"🏦 Banks to Process: {len(config['banks'])}")
    
    all_emails = fetch_all_bank_emails()
    if all_emails:
        print(f"\n🎉 Extraction completed successfully!")
        print(f"📄 Open TestEmails/index.html to browse all emails")
    else:
        print(f"\n⚠️ No emails found from any configured banks")