...
- `EMAIL_USER` — Gmail address for IMAP (optional; if unset, email features are disabled)
- `EMAIL_PASS` — Gmail app password for IMAP (optional; if unset, email features are disabled)
...
- **Deployment (production):** `NODE_ENV=production` is set via `.replit` `[userenv.production]`. Email credentials are optional — startup succeeds without them and email extraction features stay disabled. Set `EMAIL_USER` / `EMAIL_PASS` as Replit Secrets only if you want email ingestion.
...
- **CIBC Visa Credit** (`parsers/cibc_credit.py`): handles French-format CIBC Visa Infinite (Carte Dividendes) credit card statements. Three transaction sections: payments (`Vos paiements`), interest (`Vos intérêts`), purchases (`Vos nouveaux frais et crédits`). Strips CIBC spending categories (Transports, Restaurants, etc.) and interest rate columns from transaction lines. Handles `Ý` reward multiplier markers. French month abbreviations, comma-decimal amounts.
- **Capital One Mastercard Credit** (`parsers/capital_one_credit.py`): handles English-format Capital One Mastercard credit card statements. Three sections: Payments/Credits/Adjustments, Transactions, Other Charges (interest). `Mon DD` dates (year inferred from statement period). Handles multi-line FX transactions with USD conversion rates (`28.74 USD @ 1.417*`). Validates against summary totals.
- **MBNA Mastercard Credit** (`parsers/mbna_credit.py`): handles English-format MBNA Mastercard credit card statements. Three sections: PAYMENTS, PURCHASES, INTEREST CHARGED. MM/DD/YY dates, `$XX.XX` amounts (negative for payments), 4-digit reference numbers stripped from descriptions. Handles multi-line FX transactions (e.g., Amazon EUR purchases with foreign amount on continuation line). Validates against summary totals.
...
- **Neo Financial Credit** (`parsers/neo_credit.py`): handles English Neo Financial Mastercard statements (card 5135). Uses PyMuPDF (fitz) instead of pdfplumber because Neo PDFs use font obfuscation that maps digit glyphs to null bytes. Parses 4-line transaction blocks (tx_date, posted_date, description, amount). Negative amounts = purchases, positive = payments. Shared logic in `parsers/neo_base.py`.
- **Neo Financial World Elite** (`parsers/neo_credit_world_elite.py`): handles Neo Financial World Elite Mastercard statements (card 5948). Same parsing logic as neo_credit (via shared `neo_base.py`) but tracked as a separate bank ("Neo Financial World Elite") for independent spending analysis. Detected by "World Elite" keyword in PDF text.
